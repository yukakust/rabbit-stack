#!/usr/bin/env python3
"""Run the transient QCA RAM-load artifact through its fail-closed QEMU gate."""
...
from rabbit_qca_loader import build_fetched
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="rabbit-qca-loader-qemu-") as temp_dir:
        temp = Path(temp_dir)
        image_path, variables_path = temp / "rabbit-qca-loader.img", temp / "edk2-vars.fd"
        image_path.write_bytes(image); shutil.copyfile(variables, variables_path)
        command = [
        ...
        print("QEMU has no 0CF3:E009; this proves the RAM-load path stays closed on a mismatch.")
        try:
