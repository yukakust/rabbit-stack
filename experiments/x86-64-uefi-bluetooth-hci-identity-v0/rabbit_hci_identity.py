#!/usr/bin/env python3
"""Build the bounded x86-64 UEFI Bluetooth HCI identity probe."""
...
PROGRAM_SIZE = 1952
PROGRAM_SHA256 = "63c54bfbc690a5af5094c5cc393fadbd1626071a5cc104cec215784bf01702aa"
...
def validate_probe(probe: dict[str, Any]) -> None:
    if probe != EXPECTED_PROBE:
        raise BuildError("probe differs from the reviewed single-command HCI identity contract")
    ...
    if probe["limits"] != {
        ...
        raise BuildError("HCI identity budget changed")


def validate_target(target: dict[str, Any]) -> None:
    if target != EXPECTED_TARGET:
        raise BuildError("target differs from the reviewed Dell QCA Rome identity Target Pack")
    authority = target["authority"]
    if authority["allowed_hci_opcodes"] != ["0x1001"]:
...
def parse_read_local_version_event(event: bytes) -> dict[str, int]:
    if len(event) < 14:
        raise BuildError("HCI event is shorter than Read Local Version Command Complete")
    if event[0] != 0x0E or event[1] < 12:
        raise BuildError("not a complete HCI Command Complete event")
    if event[3:5] != b"\x01\x10":
        raise BuildError("HCI event belongs to a different opcode")
    if event[5] != 0:
        raise BuildError(f"controller returned HCI status 0x{event[5]:02X}")
    return {
        "hci_version": event[6],
        "hci_revision": int.from_bytes(event[7:9], "little"),
        "lmp_version": event[9],
        "manufacturer": int.from_bytes(event[10:12], "little"),
        "lmp_subversion": int.from_bytes(event[12:14], "little"),
    }
...
def load_program() -> bytes:
    try:
        ...
        raise BuildError(f"could not decode reviewed HCI-identity bytes: {error}") from error
    if len(code) != PROGRAM_SIZE or sha256(code) != PROGRAM_SHA256:
        raise BuildError("reviewed HCI-identity machine bytes changed")
    return code
...
def build_efi(code: bytes) -> bytes:
    ...
    if raw_size != 0x800:
        raise BuildError("unexpected HCI-identity code size")
    dos = bytearray(0x80)
    dos[0:2] = b"MZ"
