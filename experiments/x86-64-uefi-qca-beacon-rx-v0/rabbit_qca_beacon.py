#!/usr/bin/env python3
"""Build one UEFI boot that initializes QCA RAM and passively receives Rabbit BLE."""
...
def validate_probe(probe: dict[str, Any]) -> None:
    if probe != EXPECTED_PROBE:
        raise BuildError("probe differs from reviewed one-boot QCA-init/passive-RX contract")


def validate_target(target: dict[str, Any]) -> None:
    if target != EXPECTED_TARGET:
        raise BuildError("target differs from reviewed combined Dell QCA Target Pack")
