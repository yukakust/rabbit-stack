#!/usr/bin/env python3
"""Run the read-only network probe under QEMU UEFI with an emulated NIC."""
...
from rabbit_network_probe import PROBE_PATH, TARGET_PATH, build, load_json
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="rabbit-network-probe-qemu-") as temp_dir:
        temp = Path(temp_dir)
        image_path = temp / "rabbit-network-probe.img"
        ...
        command = [
            ...
            "-netdev", "user,id=rabbitnet", "-device", "e1000e,netdev=rabbitnet",
        ]
        print(f"IMAGE SHA256: {report['image_sha256']}")
        print("Expected: protocol YES/NO lines and at least one PCI network controller. No packets are sent.")
        print("Photograph or transcribe the screen, then press any key. No physical device is used.")
        try:
