#!/usr/bin/env python3
"""Build the read-only x86-64 UEFI QCA Rome status probe."""
...
PROGRAM_SIZE = 1736
PROGRAM_SHA256 = "7746e8954e4a14daec9a494c730fcee7b6fe56c1e72ad02db5f1109cb16efcc0"


def _load_media_builder():
    spec = importlib.util.spec_from_file_location("rabbit_uefi_media_qca_status", MEDIA_BUILDER_PATH)
...
def validate_probe(probe):
    if probe != EXPECTED_PROBE:
        raise BuildError("probe differs from reviewed read-only QCA status contract")


def validate_target(target):
    if target != EXPECTED_TARGET:
        raise BuildError("target differs from reviewed read-only QCA status Target Pack")
    authority = target["authority"]
    if authority["vendor_in_requests"] != ["0x09:20", "0x05:1"] or authority["vendor_out_requests"] != 0:
        raise BuildError("vendor request authority changed")
...
def load_program() -> bytes:
    try:
        ...
        raise BuildError(f"could not decode reviewed status bytes: {error}") from error
    if len(code) != PROGRAM_SIZE or sha256(code) != PROGRAM_SHA256:
        raise BuildError("reviewed QCA status machine bytes changed")
    return code
...
def build_efi(code: bytes) -> bytes:
    ...
    if raw_size != 0x800:
        raise BuildError("unexpected QCA status code size")
    ...
    directories = [(0, 0)] * 16; directories[5] = (0x2000, 8)
    ...
    reloc = struct.pack("<8sIIIIIIHHI", b".reloc\0\0", 8, 0x2000, 0x200, 0x200 + raw_size, 0, 0, 0, 0, 0x42000040)
...
def build(probe, target):
    validate_probe(probe); validate_target(target)
    code = load_program(); efi = build_efi(code); image = media.build_image(efi)
    report = {
        ...
        "source_sha256": sha256(SOURCE_PATH.read_bytes()), "program_sha256": sha256(code),
        ...
        "vendor_in_requests": ["0x09:20", "0x05:1"], "vendor_out_requests": 0,
        "firmware_download_authorized": False, "controller_reset_authorized": False,
        "hci_commands_authorized": 0, "radio_operations_authorized": 0,
        "physical_execution_verified": False, "writes_performed": [],
    }
    return image, report
