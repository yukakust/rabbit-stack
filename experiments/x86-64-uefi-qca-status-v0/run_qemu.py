#!/usr/bin/env python3
"""Run the QCA status artifact through its QEMU fail-closed gate."""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="rabbit-qca-status-qemu-") as temp_dir:
        temp = Path(temp_dir)
        image_path, variables_path = temp / "rabbit-qca-status.img", temp / "edk2-vars.fd"
        ...
        print("Expected: TARGET NOT FOUND; NO VENDOR REQUEST SENT")
        print("QEMU has no 0CF3:E009; this validates the fail-closed path only.")
        try:
