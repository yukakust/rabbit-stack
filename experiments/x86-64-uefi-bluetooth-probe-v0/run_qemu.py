#!/usr/bin/env python3
"""Run the read-only USB/Bluetooth inventory under QEMU UEFI."""
...
from rabbit_bluetooth_probe import PROBE_PATH, TARGET_PATH, build, load_json
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="rabbit-bluetooth-probe-qemu-") as temp_dir:
        temp = Path(temp_dir)
        image_path = temp / "rabbit-bluetooth-probe.img"
        ...
        print("Expected: at least one USB interface; a keyboard must not be labeled Bluetooth.")
        print("QEMU does not model the Dell Bluetooth hardware. Photograph the screen, then press any key.")
        try:
