#!/usr/bin/env python3
"""Build the reviewed read-only x86-64 UEFI USB/Bluetooth inventory probe."""
...
PROGRAM_SIZE = 1152
PROGRAM_SHA256 = "1a716bd0f4f7a9eeee3b2b5cdcca2b1ecc460933d02bc33c8802d04bc142a0ce"


def _load_media_builder():
    spec = importlib.util.spec_from_file_location("rabbit_uefi_media_bluetooth_probe", MEDIA_BUILDER_PATH)
...
def validate_probe(probe: dict[str, Any]) -> None:
    if probe != EXPECTED_PROBE:
        raise BuildError("probe differs from the reviewed read-only USB/Bluetooth inventory contract")
    if probe["mutations"] != []:
        raise BuildError("USB/Bluetooth discovery must declare no mutations")
    if probe["limits"] != {"max_usb_interfaces": 64}:
        raise BuildError("USB interface budget changed")


def validate_target(target: dict[str, Any]) -> None:
    if target != EXPECTED_TARGET:
        raise BuildError("target differs from the reviewed UEFI USB/Bluetooth probe Target Pack")
    policy = target["probe_policy"]
    if policy != {
        "usb_descriptor_reads": True,
        "usb_data_transfers": False,
        "usb_port_reset": False,
        "bluetooth_hci_commands": False,
        "bluetooth_pairing": False,
        "radio_transmit": False,
        "configuration_writes": False,
    }:
        raise BuildError("probe policy permits an unreviewed USB or Bluetooth effect")
...
def load_program() -> bytes:
    try:
        ...
        raise BuildError(f"could not decode reviewed Bluetooth-probe bytes: {error}") from error
    if len(code) != PROGRAM_SIZE or sha256(code) != PROGRAM_SHA256:
        raise BuildError("reviewed Bluetooth-probe machine bytes changed")
    return code
...
def build_efi(code: bytes) -> bytes:
    ...
    if raw_size != 0x600:
        raise BuildError("unexpected Bluetooth-probe code size")
    dos = bytearray(0x80)
    dos[0:2] = b"MZ"
...
def build(probe: dict[str, Any], target: dict[str, Any]) -> tuple[bytes, dict[str, Any]]:
    ...
    report = {
        ...
        "expected_observation": "USB interfaces and standard Bluetooth class candidates",
        "usb_descriptor_reads_authorized": True,
