...
OUTPUT = Path("/tmp/rabbit-ble-color-command-v03.img")
REPORT = Path("/tmp/rabbit-ble-color-command-v03.json")
...
def main() -> int:
    try:
        ...
        if not (ROOT / "evidence" / "qemu-macos-arm64-v03-observed.json").is_file():
            raise RuntimeError("exact v0.3 QEMU mismatch gate is not recorded")
