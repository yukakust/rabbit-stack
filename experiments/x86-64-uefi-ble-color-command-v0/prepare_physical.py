...
OUTPUT = Path("/tmp/rabbit-ble-color-command-v02.img")
REPORT = Path("/tmp/rabbit-ble-color-command-v02.json")
...
def main() -> int:
    try:
        ...
        if not (ROOT / "evidence" / "qemu-macos-arm64-v02-observed.json").is_file():
            raise RuntimeError("exact v0.2 QEMU mismatch gate is not recorded")
