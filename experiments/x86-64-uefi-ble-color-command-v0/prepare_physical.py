...
OUTPUT = Path("/tmp/rabbit-ble-color-command-v04.img")
REPORT = Path("/tmp/rabbit-ble-color-command-v04.json")
...
def main() -> int:
    try:
        ...
        if not (ROOT / "evidence" / "qemu-macos-arm64-v04-observed.json").is_file():
            raise RuntimeError("exact v0.4 QEMU mismatch gate is not recorded")
